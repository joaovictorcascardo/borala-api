...
describe("Auth Service", () => {
  ...
  describe("login", () => {
    it("deve retornar usuário e token com credenciais válidas", async () => {
      ...
      const result = await AuthService.login({ email: "teste@teste.com", password: "123" });
    ...
    it("deve lançar erro com email inválido", async () => {
      ...
      await expect(AuthService.login({ email: "errado@teste.com", password: "123" }))
        .rejects.toThrow("E-mail ou senha inválidos.");
    });

    it("deve lançar erro com senha inválida", async () => {
      (UserData.prototype.login as jest.Mock).mockResolvedValue({ password_hash: "hash" });
      (bcrypt.compare as jest.Mock).mockResolvedValue(false);

      await expect(AuthService.login({ email: "teste@teste.com", password: "errada" }))
        .rejects.toThrow("E-mail ou senha inválidos.");
    });
  });
  ...
  describe("forgotPassword", () => {
    it("deve gerar token de reset se o email existir", async () => {
      ...
      const mockHash = { update: jest.fn().mockReturnThis(), digest: jest.fn().mockReturnValue("token_hash") };
      (crypto.createHash as jest.Mock).mockReturnValue(mockHash);
      (crypto.randomBytes as jest.Mock).mockReturnValue({ toString: () => "token_bruto" });

      await AuthService.forgotPassword("teste@teste.com");

      expect(UserData.prototype.updateResetToken).toHaveBeenCalledWith(1, "token_hash", expect.any(Date));
    });
    ...
    it("deve lançar erro se o email não for encontrado", async () => {
      ...
      await expect(AuthService.forgotPassword("desconhecido@teste.com"))
