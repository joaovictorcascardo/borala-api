...
class ReviewService {
  ...
  async getReviewsById(
    ...
    try {
      const reviews = await this.reviewData.getReviews(userId, page, limit);
      if (!reviews) {
        if (page === 1) {
          throw new Error(
            "Nenhuma avaliação foi encontrada para este usuário."
          );
        } else {
  ...
  async createReview(reviewData: CreateReviewDTO): Promise<CreateReviewDTO[]> {
    ...
    const isReviewerPassenger = await this.bookingData.isPassenger(
      reviewData.ride_id,
      reviewData.reviewer_id
    );
    if (!isReviewerDriver && !isReviewerPassenger) {
      throw new Error(
        "Você não participou desta carona, então não pode avaliá-la."
      );
    }

    const isRevieweeDriver = ride.driver_id == reviewData.reviewee_id;
    const isRevieweePassenger = await this.bookingData.isPassenger(
      reviewData.ride_id,
    ...
    if (!isRevieweeDriver && !isRevieweePassenger) {
      throw new Error(
        "O usuário que você quer avaliar não participou desta carona."
      );
    }

    const existingReview = await this.reviewData.searchReview(
      reviewData.ride_id,
      reviewData.reviewer_id,
      reviewData.reviewee_id
    );
