...
describe("Booking Controller", () => {
  ...
  const setupScenario = async () => {
    ...
    const driverLogin = await request(app)
      ...
      .send({ email: "mot@t.com", password: "123456" });
    ...
    const rideRes = await request(app)
      ...
      .set("Authorization", `Bearer ${driverToken}`)
      .send({
        vehicle_id: Number(vehicleRes.body.id), // Garantir número
        origin_address: "Origem A",
        origin_latitude: 0,
        origin_longitude: 0,
    ...
    // CORRIGIDO: senha "123456"
    const passengerRes = await request(app).post("/users").send({
      name: "Passageiro",
      email: "pass@t.com",
      password: "123456",
      birth_date: "2000-01-01",
    ...
    const passengerLogin = await request(app)
      .post("/authenticator/sessions")
      .send({ email: "pass@t.com", password: "123456" });
  ...
  describe("POST /rides/:rideId/bookings", () => {
    ...
    it("deve falhar se o motorista tentar reservar a própria viagem", async () => {
      ...
      expect(response.body.message).toContain(
        "não pode reservar sua própria viagem"
      );
    });
  });
